...
def auto_seed_if_empty():
    ...
    db = SessionLocal()
    try:
        # Check if the User table has 0 rows
        user_count = db.query(models.User).count()
        if user_count == 0:
            print("Database is empty. Executing auto-seeding for demo data...")
            seed_data(db)
        else:
            print(f"Database already contains data ({user_count} users). Skipping auto-seed.")
    except Exception as e:
